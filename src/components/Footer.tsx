    ...
    <footer className="w-full bg-[#030303] border-t border-white/5 py-12 px-6 md:px-12 lg:px-24 text-xs font-mono text-[#8c8c8c]">
      <div className="mx-auto max-w-7xl flex flex-col sm:flex-row items-center justify-between space-y-6 sm:space-y-0">
        ...
        <div>
          <button
            onClick={scrollToTop}
            className="group flex items-center space-x-2 border border-white/10 hover:border-white hover:text-white rounded-full py-2 px-4 text-white transition-all duration-300 focus:outline-none cursor-pointer"
            id="back-to-top-btn"
          >
            <span className="text-[9px] tracking-widest uppercase">TERUG NAAR BOVEN</span>
            <ArrowUp className="h-3.5 w-3.5 text-white/60 transition-transform duration-300 group-hover:-translate-y-0.5" />
          </button>
        </div>
      </div>
    </footer>
